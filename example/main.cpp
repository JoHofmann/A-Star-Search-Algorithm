...
int main()
{
	...
	while (window.isOpen()) {
		...
		drawMap[start.x][start.y] = 5;
		drawMap[target.x][target.y] = 5;
		...
		for (uint y = 0; y < height/cellSize; ++y) {
			for (uint x = 0; x < width/cellSize; ++x) {
				...
				rect.setPosition(x * cellSize, y * cellSize);
			    	...
			    	else if(drawMap[x][y] == 2) {	// openList
			    		rect.setFillColor(sf::Color::Yellow);
			    	}
			    	else if(drawMap[x][y] == 3) {	// closedList
			    		rect.setFillColor(sf::Color::Red);
			    	}
			       	else if(drawMap[x][y] == 4) {	// current path
					rect.setFillColor(sf::Color::Cyan);
				}
				else if(drawMap[x][y] == 5) {	// start/target
